...
export class FabricGuide extends Disposable {
  ...
  private objectMoving({ target }: any) {
    ...
    // console.log("===>target.parent", target.type, target.getAncestors(true));
    // return;

    if (check.isActiveSelection(target)) {
      const needAddParent = new Set<Group | Canvas | StaticCanvas>();
      target.forEachObject((obj) => {
        const parent = obj.getAncestors(true);
        if (parent && parent.length > 0) {
          (parent || []).forEach((ele) => {
            needAddParent.add(ele as any);
          });
        }
      });
      needAddParent.forEach((p) => {
        if (check.isNativeGroup(p)) {
          canvasObjects.push(p);
        }
        add(p);
      });
    } else {
      const parent = target.getAncestors(true) as Group[];
      if (parent && parent.length > 0) {
        (parent || []).forEach((p) => {
          if (check.isNativeGroup()) {
            canvasObjects.push(p);
          }
          add(p);
        });
