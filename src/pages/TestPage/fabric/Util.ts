...
fabric.util.getLocalPoint = function (
...
fabric.util.commonEventInfo = function (
  eventData: Event,
  transform: fabric.Transform,
  x: number,
  y: number
...
fabric.util.wrapWithFireEvent = function (
  eventName: string,
  ...
  return function (
    ...
    if (actionPerformed) {
      fabric.util.fireEvent(
        eventName,
        fabric.util.commonEventInfo(eventData, transform, x, y)
      );
    }
...
fabric.util.changeSize = function (
  eventData: Event,
  transform: fabric.Transform,
  x: number,
  y: number
  ...
  let target = transform.target,
    ...
    localPoint = fabric.util.getLocalPoint(
      transform,
      transform.originX,
      transform.originY,
      x,
...
fabric.controlsUtils.changeSize = fabric.util.wrapWithFireEvent(
  "resizing",
  fabric.util.wrapWithFixedAnchor(fabric.util.changeSize)
);
