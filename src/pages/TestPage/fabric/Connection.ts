...
class FabricConnection extends fabric.Line {
  ...
  constructor(options: Partial<fabric.ConnectionOptions>) {
    ...
    this._checkIfWatchedObjectAddedInSelection = () =>
      this.checkIfWatchedObjectAddedInSelection();
    this._remove = () => {
  ...
  checkIfWatchedObjectAddedInSelection() {
    if (this.canvas._activeObject?.constructor === fabric.ActiveSelection) {
      ...
      let shouldWatch = !!selection._objects.find(
        (obj) => this._target || this._source
      );
      if (shouldWatch) {
        selection.on({
          moving: this._update,
  ...
  _setSource(object: fabric.Object | null) {
    if (this._source) {
      ...
      this._source._outputConections.splice(
        this._source._outputConections.indexOf(this),
        1
      );
      if (this._target?.id && this._source.connections) {
        this._source.connections.splice(
          this._source.connections.indexOf(this._target.id),
          1
        );
      }
    }
  ...
  _setTarget(object: fabric.Object | null) {
    if (this._target) {
      ...
      this._target._inputConections.splice(
        this._target._inputConections.indexOf(this),
        1
      );
  ...
  override _render(ctx: CanvasRenderingContext2D) {
    ...
    switch (this.orientation) {
      case "horizontal": {
        ...
        let arrowSize = Math.max(
          0,
          Math.min(this.cornerSize || 1, Math.abs(w) / 2 - 4)
        );
      ...
      case "vertical": {
        ...
        let arrowSize = Math.max(
          0,
          Math.min(this.cornerSize || 1, Math.abs(h) / 2 - 4)
        );
