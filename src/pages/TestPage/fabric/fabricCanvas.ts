...
fabric.Canvas.prototype._handModeMouseDown = function (
...
fabric.Canvas.prototype.setOriginalSize = function (
  w: number | HTMLImageElement,
  h: number
) {
...
fabric.Canvas.prototype._zoomToPointNative =
...
fabric.Canvas.prototype.zoomToPoint = function (
  point: fabric.IPoint,
  newZoom: number
) {
...
fabric.Canvas.prototype.wheelZoom = function (
...
fabric.Canvas.prototype.getOrignalCenter = function () {
  return {
    ...
    y:
      (this.height / 2) * this.viewportTransform[3] + this.viewportTransform[5],
  };
};
fabric.Canvas.prototype.updateScrollbars = function () {
  ...
  this.horizontalScrollElementBg.style.width =
    this.originalWidth * zoom + this.width + "px";
  this.verticalScrollElementBg.style.height =
    this.originalHeight * zoom + this.height + "px";
};
...
fabric.Canvas.prototype.createScrollbars = function () {
  ...
  this.wrapperEl.addEventListener("mousewheel", (event) =>
    this._scrollEvent(event as WheelEvent)
  );
  ...
  this.horizontalScrollElement.onscroll = this.verticalScrollElement.onscroll =
    (e) => {
      this.viewportTransform[4] =
        -this.horizontalScrollElement.scrollLeft + this.width / 2;
      this.viewportTransform[5] =
        -this.verticalScrollElement.scrollTop + this.height / 2;
      this.renderAll();
    };
};
